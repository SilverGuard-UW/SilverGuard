...
const translations = {
  en: {
    ...
    "login.passwordPlaceholder": "Enter your password (min. 6 characters)",
    "login.loginButton": "Log In",
    "login.signingIn": "Signing in...",
    "login.hint": "New here? Enter a username and password to create your account.",
    "login.error.emptyUsername": "Please enter a username.",
    "login.error.emptyPassword": "Please enter a password.",
  ...
  es: {
    ...
    "login.passwordPlaceholder": "Ingrese su contraseña (mín. 6 caracteres)",
    "login.loginButton": "Iniciar Sesión",
    "login.signingIn": "Iniciando sesión...",
    "login.hint": "¿Nuevo aquí? Ingrese un nombre de usuario y contraseña para crear su cuenta.",
    "login.error.emptyUsername": "Por favor ingrese un nombre de usuario.",
    "login.error.emptyPassword": "Por favor ingrese una contraseña.",
    "login.error.shortPassword": "La contraseña debe tener al menos 6 caracteres.",
    "login.error.wrongPassword": "Contraseña incorrecta. Por favor inténtelo de nuevo.",
    "login.error.generic": "Algo salió mal. Por favor inténtelo de nuevo.",
...
export function AccessibilityProvider({ children }: { children: ReactNode }) {
  const [language, setLanguageState] = useState(() => {
    return localStorage.getItem(getUserKey("silverguard-language")) || "english";
  });

  const [textSize, setTextSizeState] = useState(() => {
    const saved = localStorage.getItem(getUserKey("silverguard-textsize"));
    return saved ? parseInt(saved) : 100;
  });

  const setLanguage = (lang: string) => {
    setLanguageState(lang);
    localStorage.setItem(getUserKey("silverguard-language"), lang);
  };

  const setTextSize = (size: number) => {
    setTextSizeState(size);
    localStorage.setItem(getUserKey("silverguard-textsize"), size.toString());
  };

  const reloadForUser = () => {
    const lang = localStorage.getItem(getUserKey("silverguard-language")) || "english";
    const saved = localStorage.getItem(getUserKey("silverguard-textsize"));
    setLanguageState(lang);
    setTextSizeState(saved ? parseInt(saved) : 100);
  };
